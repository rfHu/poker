public class AchieveModel
{
    public string uid;
    public string coins;
    public string score;
    public int total_match_count;
    public int total_bankroll;
    public int total_bankroll_return;
    public string total_hand_count;
    public int entry_hand_count;
    public float entry_hand_percent;
    public float showdown_hand_percent;
    public float entry_win_hand_percent;
    public float aggressiveness;
    public float pfr_hand_percent;
    public float t_bet_percent;
    public float c_bet_round_percent;
    public float bankroll_return_percent;
    public int pfr_hand_count;
    public string win_hand_count;
    public int pf_raise_count;
    public int pf_bet_count;
    public int pf_call_count;
    public int seecard_hand_count;
    public int showdown_hand_count;
    public string total_allin_count;
    public string allin_win_count;
    //SNG
    public int win_match_count;
    public int golden_cup_count;
    public int silver_cup_count;
    public int copper_cup_count;    
    public float win_match_percent;
}